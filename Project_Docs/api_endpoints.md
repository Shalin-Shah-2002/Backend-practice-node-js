...
| `POST` | `/auth/register` | Register a new user (sign‑up). | Enables a new account creation and returns a JWT for immediate access. |
| `GET` | `/api/users` | List all users. | Returns the public user directory for any authenticated caller. |
| `GET` | `/api/users/:id` | Retrieve a single user’s public profile. | Used by an authenticated client to view a specific user by ID. |
| `PUT` | `/api/users/:id` | Update a user’s information. | Allows the authenticated owner to modify profile data, e.g., email or password. |
| `DELETE` | `/api/users/:id` | Delete a user account. | Enables account removal for the authenticated owner. |
